...
describe("FolderTransferPanel", () => {
  ...
  it("renders fab and badge correctly", () => {
    useFolderTransferStore.setState({
      folderTransfers: [
        { id: "1", folderName: "active_folder", totalSize: 100, totalFiles: 2, transferredBytes: 50, transferredFiles: 1, speed: 10, state: "active", files: [] },
        { id: "2", folderName: "queued_folder", totalSize: 200, totalFiles: 3, transferredBytes: 0, transferredFiles: 0, speed: 0, state: "queued", files: [] },
        { id: "3", folderName: "done_folder", totalSize: 300, totalFiles: 4, transferredBytes: 300, transferredFiles: 4, speed: 0, state: "completed", files: [] },
      ],
    });
  ...
  it("toggles panel visibility", () => {
    useFolderTransferStore.setState({
      folderTransfers: [{ id: "1", folderName: "f", totalSize: 10, totalFiles: 1, transferredBytes: 1, transferredFiles: 0, speed: 5, state: "active", files: [] }],
    });
  ...
  describe("FolderTransferPanel.progress", () => {
    it("displays correct progress, speed, and status for different states", () => {
      useFolderTransferStore.setState({
        folderTransfers: [
          { id: "t1", folderName: "f1", totalSize: 1024, totalFiles: 2, transferredBytes: 512, transferredFiles: 1, speed: 512, state: "active", files: [] },
          { id: "t2", folderName: "f2", totalSize: 2048, totalFiles: 3, transferredBytes: 2048, transferredFiles: 3, speed: 0, state: "completed", files: [] },
        ],
      });
  ...
  describe("FolderTransferPanel.cancel", () => {
    it("calls invoke with cancel_folder_transfer on cancel button click", async () => {
      useFolderTransferStore.setState({
        folderTransfers: [
          { id: "t1", folderName: "f1", totalSize: 100, totalFiles: 1, transferredBytes: 10, transferredFiles: 0, speed: 10, state: "active", files: [] },
        ],
      });
