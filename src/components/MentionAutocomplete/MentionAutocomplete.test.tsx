...
describe('MentionAutocomplete', () => {
  it('renders empty state when no matches', () => {
    render(
      <MentionAutocomplete query="xyz" peers={mockPeers} selectedIndex={0} onSelect={vi.fn()} />
    );
    expect(screen.getByText('No matches found')).toBeInTheDocument();
  });

  it('filters peers based on query and sorts online first', () => {
    render(<MentionAutocomplete query="" peers={mockPeers} selectedIndex={0} onSelect={vi.fn()} />);
  ...
  it('filters by query correctly', () => {
    render(
      <MentionAutocomplete query="c" peers={mockPeers} selectedIndex={0} onSelect={vi.fn()} />
    );
  ...
  it('calls onSelect when an item is clicked', () => {
    ...
    render(
      <MentionAutocomplete query="" peers={mockPeers} selectedIndex={0} onSelect={handleSelect} />
    );
