...
export const MessageInput: React.FC<MessageInputProps> = ({ peerId, onSend, disabled }) => {
      ...
      if (filteredPeers.length > 0) {
        ...
        if (e.key === 'ArrowUp') {
          ...
          setMentionSelectedIndex(
            (prev) => (prev - 1 + filteredPeers.length) % filteredPeers.length
          );
          return;
        }
        if (e.key === 'Enter') {
  ...
  const handleDrop = useCallback(
    async (event: DragEvent) => {
      ...
      if (event.dataTransfer?.files && event.dataTransfer.files.length > 0) {
        const filePaths = Array.from(event.dataTransfer.files).map((file) => file.name);
  ...
  return (
    <div ref={containerRef} className={styles.container} data-testid="message-input-container">
      {isDragging && (
        <div className={styles.dropOverlay}>{t('chat.messageInput.dropToSendFiles')}</div>
      )}
        ...
        <div className={styles.replyPreviewBar} data-testid="reply-preview-bar">
          <div className={styles.replyPreviewContent}>
            <span className={styles.replyPreviewLabel}>
              {t('chat.messageInput.replyingTo', { senderName: replyingTo.senderName || '' })}
            </span>
          ...
          <button
            ...
            aria-label={t('chat.messageInput.cancelReply')}
      ...
      <div className={styles.inputWrapper}>
        ...
        {isNearLimit && (
          <div
            className={`${styles.charCount} ${charsRemaining === 0 ? styles.charCountLimit : ''}`}
          >
            {charsRemaining}
          </div>
        )}
